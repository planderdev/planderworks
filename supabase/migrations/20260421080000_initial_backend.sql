...
to authenticated
using (created_by = auth.uid() or public.current_user_role() in ('admin', 'manager'))
with check (created_by = auth.uid() or public.current_user_role() in ('admin', 'manager'));
...
drop policy if exists "activity logs are readable to managers" on public.activity_logs;
create policy "activity logs are readable to managers"
